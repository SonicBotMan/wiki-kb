...
Wiki KB 是一个标准 MCP Server。任何支持 MCP 的 Agent（Claude Desktop、Cursor、Hermes、OpenHands 等）都能直接调用它的 16 个 tools。不需要 SDK、不需要适配器、不需要改 Agent 代码。
...
│   ├── wiki_mcp_server.py      # MCP Server 主程序（16 个 tools）
│   ├── wiki_config.py          # 集中配置模块（dataclass + 环境变量层级）
│   ├── wiki_utils.py           # 共享工具模块（frontmatter 解析、relations 表）
...
| `wiki_health` | 健康检查（registry 完整性、磁盘、OpenViking 连通性） |
...
| `NOTION_DB_TOOL` | Notion Sync | — | Notion Tool 数据库 ID |
| `NOTION_DB_PROJECT` | Notion Sync | — | Notion Project 数据库 ID |
